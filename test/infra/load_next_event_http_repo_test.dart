...
class LoadNextEventHttpRepository {
  ...
  Future<void> loadNextEvent({required String groupId}) async {
    final uri = url.replaceFirst(':groupId', groupId);
    final headers = {'content-type': 'application/json', 'accept': 'application/json'};
    await httpClient.get(Uri.parse(uri), headers: headers);
  }
}
...
void main() {
  ...
  setUpAll(() {
    url = 'http://domain.com/groups/:groupId/next_event';
  });
  ...
  test('should request with correct url', () async {
    await sut.loadNextEvent(groupId: groupId);
    expect(httpClient.url, 'http://domain.com/groups/$groupId/next_event');
  });
